...
function createInjectedTypeInstance(type, props, internalInstanceHandle, applyDisplayObjectProps) {
  ...
  if (type in INJECTED_TYPES) {
    ...
    var customDisplayObject = void 0;
...
function CustomPIXIComponent(behavior, type) {
  invariant_1(typeof type === "string", "Invalid argument `type` of type `%s` supplied to `CustomPIXIComponent`, expected `string`.", typeof type === "undefined" ? "undefined" : _typeof(type));
...
var license = "MIT";
var repository = "github:michalochman/react-pixi-fiber";
var typings = "./index.d.ts";
var files = ["LICENSE", "README.md", "index.d.ts", "index.js", "react-pixi-alias.js", "cjs/", "src/"];
var dependencies = { "fbjs": "^0.8.0", "performance-now": "^2.1.0", "react-reconciler": "^0.12.0" };
var peerDependencies = { "pixi.js": "^4.4.0", "react": "^16.0.0", "react-dom": "^16.0.0" };
var devDependencies = { "@types/pixi.js": "^4.7.3", "@types/react": "^16.0.0", "babel-core": "^6.26.3", "babel-eslint": "^8.2.3", "babel-jest": "^22.4.3", "babel-plugin-external-helpers": "^6.22.0", "babel-plugin-rewire": "^1.1.0", "babel-plugin-transform-es2015-modules-commonjs": "^6.26.2", "babel-plugin-transform-object-rest-spread": "^6.26.0", "babel-plugin-transform-react-jsx": "^6.24.1", "babel-preset-env": "^1.7.0", "canvas-prebuilt": "^1.6.5-prerelease.1", "codecov": "^3.0.2", "eslint": "^4.19.1", "eslint-config-prettier": "^2.9.0", "eslint-plugin-babel": "^5.1.0", "eslint-plugin-import": "^2.11.0", "eslint-plugin-prettier": "^2.6.0", "eslint-plugin-promise": "^3.7.0", "eslint-plugin-react": "^7.7.0", "jest": "^22.4.3", "pixi.js": "^4.4.0", "prettier": "^1.12.1", "react": "^16.0.0", "react-dom": "^16.0.0", "react-test-renderer": "^16.0.0", "rollup": "^0.58.2", "rollup-plugin-babel": "^3.0.4", "rollup-plugin-commonjs": "^9.1.3", "rollup-plugin-json": "^2.3.0", "rollup-plugin-node-globals": "^1.2.1", "rollup-plugin-node-resolve": "^3.3.0", "rollup-plugin-replace": "^2.0.0", "rollup-plugin-uglify": "^3.0.0", "rollup-plugin-visualizer": "^0.6.0", "typescript": "^2.9.2" };
var scripts = { "build": "npm run build:prod && npm run build:dev", "build:dev": "NODE_ENV=development rollup -c config/rollup.config.js", "build:prod": "NODE_ENV=production rollup -c config/rollup.config.js", "build:alias:dev": "NODE_ENV=development rollup -c config/rollup.config.js --environment entry:alias", "build:alias:prod": "NODE_ENV=production rollup -c config/rollup.config.js --environment entry:alias", "build:index:dev": "NODE_ENV=development rollup -c config/rollup.config.js --environment entry:index", "build:index:prod": "NODE_ENV=production rollup -c config/rollup.config.js --environment entry:index", "eslint": "eslint src", "eslint-check": "eslint --print-config .eslintrc.json | eslint-config-prettier-check", "prepublishOnly": "npm run test && npm run build", "test": "jest", "test:ts": "tsc -p tsconfig.json" };
var jest = { "coverageDirectory": "coverage", "coverageReporters": ["lcov", "text-summary"], "collectCoverageFrom": ["src/**/*.js"], "globals": { "true": true }, "setupFiles": ["./test/jest.setupPixi.js"], "transform": { "^.+\\.js$": "babel-jest" } };
var pkg = {
  name: name,
  version: version,
...
var RESERVED_PROPS = defineProperty({}, CHILDREN, true);
...
function setPixiValue(instance, propName, value) {
  if (isPointType(instance[propName]) && isPointType(value)) {
    ...
    invariant_1(typeof coordinateData !== "undefined" && coordinateData.length > 0 && coordinateData.length < 3, "The property `%s` is a PIXI.Point or PIXI.ObservablePoint and must be set to a comma-separated string of " + "either 1 or 2 coordinates, a 1 or 2 element array containing coordinates, or a PIXI Point/ObservablePoint. " + "If only one coordinate is given then X and Y will be set to the provided value. Received: `%s` of type `%s`.", propName, JSON.stringify(value), typeof value === "undefined" ? "undefined" : _typeof(value));
...
function commitUpdate(instance, updatePayload, type, lastRawProps, nextRawProps, internalInstanceHandle) {
  ...
  var newProps = filterByKey(nextRawProps, updatedPropKeys);
...
var ReactPixiFiberAsPrimaryRenderer = reactReconciler(_extends({}, hostConfig, { isPrimaryRenderer: true }));

// React Pixi Fiber renderer is secondary to React DOM renderer if used together
var ReactPixiFiberAsSecondaryRenderer = reactReconciler(_extends({}, hostConfig, { isPrimaryRenderer: false }));
...
function validateCanvas(props, propName, componentName) {
  ...
  if (!isCanvas) {
    var propType = typeof element === "undefined" ? "undefined" : _typeof(element);
    return new Error("Invalid prop '" + propName + "' of type '" + propType + "' supplied to '" + componentName + "', expected '<canvas> Element'.");
  }
}
...
var Stage = function (_React$Component) {
  inherits(Stage, _React$Component);

  function Stage() {
    classCallCheck(this, Stage);
    return possibleConstructorReturn(this, (Stage.__proto__ || Object.getPrototypeOf(Stage)).apply(this, arguments));
  }
  ...
  createClass(Stage, [{
    ...
    value: function componentDidMount() {
      ...
      this._app = new PIXI.Application(_extends({
        height: height,
        width: width,
        view: this._canvas
      }, options));
      ...
      // - callback
      // - return value
      this._mountNode = ReactPixiFiberAsSecondaryRenderer.createContainer(this._app.stage);
      ReactPixiFiberAsSecondaryRenderer.updateContainer(children, this._mountNode, this);

      ReactPixiFiberAsSecondaryRenderer.injectIntoDevTools({
        findFiberByHostInstance: ReactPixiFiberAsSecondaryRenderer.findFiberByHostInstance,
        bundleType: 1,
        version: pkg.version,
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      var currentHeight = options && options.height || height;
      var currentWidth = options && options.width || width;
      var prevHeight = prevOptions && prevOptions.height || prevProps.height;
      var prevWidth = prevOptions && prevOptions.width || prevProps.width;
    ...
    value: function componentWillUnmount() {
      ReactPixiFiberAsSecondaryRenderer.updateContainer(null, this._mountNode, this);
      this._app.destroy();
    }
  }, {
    key: "render",
    value: function render() {
      ...
      if (typeof options !== "undefined" && options.view) {
        ...
        return React.createElement("canvas", _extends({ ref: function ref(_ref) {
...
function render(element, containerTag, callback) {
  ...
  if (!root) {
    root = ReactPixiFiberAsPrimaryRenderer.createContainer(containerTag);
    roots.set(containerTag, root);
  }

  ReactPixiFiberAsPrimaryRenderer.updateContainer(element, root, undefined, callback);

  ReactPixiFiberAsPrimaryRenderer.injectIntoDevTools({
    findFiberByHostInstance: ReactPixiFiberAsPrimaryRenderer.findFiberByHostInstance,
    bundleType: 1,
    version: pkg.version,
    rendererPackageName: pkg.name
  });

  return ReactPixiFiberAsPrimaryRenderer.getPublicRootInstance(root);
}
