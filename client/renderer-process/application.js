...
exports.init = function() {
    ...
    for (let i = 0; i < 9; i++) {
        ...
        if (project.project.hotbar[i]) {
            ...
            element.style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/${project.project.hotbar[i]}.png?random=${new Date().getTime()})`
        }
    }
    ...
    for (let i = 0; i < character.emotes.length; i++) {
        ...
        if (character.emotes[i].enabled) {
            ...
            if (fs.existsSync(path.join(project.charactersPath, '..', 'thumbnails', `new-${character.id}`, `${i}.png`)))
                document.getElementById(i).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${character.id}/${i}.png?random=${new Date().getTime()})`
            else
                document.getElementById(i).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/${character.id}/${i}.png?random=${new Date().getTime()})`
...
exports.updateCharacter = function(character, updateThumbnail) {
    ...
    if (index > -1) {
        ...
        if (updateThumbnail) {
            ...
            document.getElementById(`char ${index}`).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${character.id}.png?random=${new Date().getTime()})`
        }
            ...
            if (updateThumbnail) {
                ...
                for (let i = 0; i < emotes.length; i++) {
                    if (character.emotes[emotes[i]] && character.emotes[emotes[i]].enabled)
                        document.getElementById(emotes[i]).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${character.id}/${i}.png?random=${new Date().getTime()})`
                }
            }
        }
    }
    if (updateThumbnail) {
        ...
        if (fs.existsSync(path.join(project.charactersPath, '..', 'thumbnails', `new-${character.id}.png`)))
            charPath = `${project.charactersPath}/../thumbnails/new-${character.id}.png?random=${new Date().getTime()}`
        else charPath = `${project.charactersPath}/../thumbnails/new-${character.id}.png?random=${new Date().getTime()}`
...
function charContextMenu(e) {
    ...
    if (project.project.hotbar[i]) {
        if (fs.existsSync(path.join(project.charactersPath, '..', 'thumbnails', `new-${project.project.hotbar[i]}.png`)))
            document.getElementById('char selected').style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${project.project.hotbar[i]}.png?random=${new Date().getTime()})`
        else
            document.getElementById('char selected').style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/${project.project.hotbar[i]}.png?random=${new Date().getTime()})`
    } else {
...
function addCharSelector(character, i) {
    ...
    if (fs.existsSync(path.join(project.charactersPath, '..', 'thumbnails', `new-${character}.png`)))
        selector.style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${character}.png?random=${new Date().getTime()})`
    else
        selector.style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/${character}.png?random=${new Date().getTime()})`
...
function updateHotbar(e) {
    ...
    if (project.project.hotbar[i] && project.project.hotbar[i] !== 0) {
        ...
        if (fs.existsSync(path.join(project.charactersPath, '..', 'thumbnails', `${puppet}.png`)))
            document.getElementById(`char ${i}`).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/${puppet}.png?random=${new Date().getTime()})`
        else
            document.getElementById(`char ${i}`).style.backgroundImage = `url(${project.charactersPath.replace(/\\/g, '/')}/../thumbnails/new-${puppet}.png?random=${new Date().getTime()})`
    } else {
...
function startAutocrop() {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        asset.style.backgroundImage = `url(${project.assetsPath}/${project.assets[keys[i]].location}?random=${new Date().getTime()})`.replace(/\\/g, '/')
...
function autocrop() {
    ...
    image.onload = () => {
        ...
        try {
            ...
            document.getElementById(`autocrop-${assets[i].id}`).style.backgroundImage = `url(${project.assetsPath}/${assets[i].location}?random=${new Date().getTime()})`.replace(/\\/g, '/')
