                ...
                let readAssetList = function(name, list) {
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        ...
                        if (list[keys[i]].type === 'animated') {
                            ...
                            location = [location.slice(0, location.length - 4), '.thumb', location.slice(location.length - 4)].join('')
                            asset.style.backgroundImage = `url(${filepaths[0].replace(/\\/g, '/')}/../assets/${location}?random=${new Date().getTime()})`
                            asset.className += ' animated'
                        } else 
                            asset.style.backgroundImage = `url(${filepaths[0].replace(/\\/g, '/')}/../assets/${list[keys[i]].location}?random=${new Date().getTime()})`
...
function assetType(e) {
    ...
    if (e.target.value.toLowerCase() === 'animated') {
        ...
        document.getElementById('asset selected').style.background = `url(${project.assetsPath.replace(/\\/g, '/')}/${location.replace(/\\/g, '/')}?random=${new Date().getTime()}) center no-repeat/contain`
        ...
        document.getElementById('asset selected').style.background = `url(${project.assetsPath.replace(/\\/g, '/')}/${asset.location.replace(/\\/g, '/')}?random=${new Date().getTime()}) center no-repeat/contain`
...
function recreateThumb(asset) {
    ...
    image.onload = () => {
        ...
        fs.writeFile(path.join(project.assetsPath, location), new Buffer(canvas.toDataURL().replace(/^data:image\/\w+;base64,/, ''), 'base64'), (err) => {
            if (err) console.log(err)
            document.getElementById('asset selected').style.background = `url(${project.assetsPath.replace(/\\/g, '/')}/${location.replace(/\\/g, '/')}?random=${new Date().getTime()}) center no-repeat/contain`
            document.getElementById(asset.name.toLowerCase()).children[1].src = `${project.assetsPath.replace(/\\/g, '/')}/${location.replace(/\\/g, '/')}`
        })
    }
